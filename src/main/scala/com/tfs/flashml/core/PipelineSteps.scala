...
object PipelineSteps {
  ...
  def run(): Unit = {
    ...
    log.info(s"Time till acquiring vectorized data: ${
    ...
    log.info(s"Time till training/loading model(s): ${
      TimeUnit.NANOSECONDS.toSeconds(System.nanoTime() -
        startTime)
    } sec")
    ...
    log.info(s"Time till prediction/loading prediction data: ${
      TimeUnit.NANOSECONDS.toSeconds(System.nanoTime() -
        startTime)
    } sec")
    ...
    if (steps.contains(FlashMLConstants.PUBLISH)) {
      ...
      format match {
        ...
        case FlashMLConstants.PUBLISH_MLEAP => Publish.generateMleap(if (ConfigUtils.isPageLevelModel) pageLevelDataFrameArray else samplingDF)
