...
import com.tfs.flashml.core.featuregeneration.transformer.{CategoricalColumnsTransformer, GramAssembler,
    SkipGramGenerator}
...
object FeatureGenerationEngine extends Engine with Validator
{
        ...
        odfArray.map(dfArray =>
        {
            if (ConfigUtils.isModel)
            {
                ...
                if (FlashMLConfig
                        ...
                        .unwrapped()
                        .asInstanceOf[java.util.ArrayList[Any]].isEmpty && ConfigUtils.categoricalColumns
                        .asInstanceOf[Array[Any]].isEmpty)
                    return Some(dfArray)
                else
                {
                    if (FlashMLConfig
                    ...
                    (FlashMLConstants.EXPERIMENT_FEATURE_GENERATION_GRAMS)
                            .unwrapped()
                            .asInstanceOf[java.util.ArrayList[Any]].nonEmpty)
                    {
        ...
        if (featureGenerationBinningConfig.nonEmpty)
        {
            ...
            ConfigUtils.featureGenerationScope match
            {
                ...
                case FlashMLConstants.SCOPE_PARAMETER_ALL_PAGE | FlashMLConstants.SCOPE_PARAMETER_NO_PAGE =>
                    ...
                    log.info(s"Feature Generation: Adding Binning transformer for page number " + pageNum + " to the " +
                            "pipeline")
                    allStages ++= getBinningPipelineStages(featureGenerationBinningConfig.asInstanceOf[Array[util
                    .HashMap[String, Any]]], df, pageNum)

                case FlashMLConstants.SCOPE_PARAMETER_PER_PAGE =>
                    if (featureGenerationBinningConfig.isDefinedAt(pageNum) && featureGenerationBinningConfig
                    (pageNum).asInstanceOf[Array[util.HashMap[String, Any]]].nonEmpty)
                    {
                        val pageSpecificBinningConfig = featureGenerationBinningConfig(pageNum)
                                .asInstanceOf[Array[util.HashMap[String, Any]]]

                        log.info(s"Feature Generation: Adding Binning transformer for page number " + pageNum + " to " +
                                "the pipeline")
        ...
        ConfigUtils.featureGenerationScope match
        {
            case FlashMLConstants.SCOPE_PARAMETER_NO_PAGE =>
                        ...
                        .asInstanceOf[Array[util.HashMap[String, Any]]].nonEmpty)
                {
                    ...
                    val featureGenerationMapArray = featureGenerationGramsConfig.asInstanceOf[Array[util
                    .HashMap[String, Any]]]
     ...
     * @param pageNum                   the PageNumber on which Binning is operated. Reqd. for labelling the output
     *                                  binned column
     * @return Array of Pipeline stages for each of the input binning variables for the given page
     */
    private def getBinningPipelineStages(pageSpecificBinningConfig: Array[java.util.HashMap[String, Any]],
                                         df: DataFrame, pageNum: Int): Array[PipelineStage] =
    {
        ...
        pageSpecificBinningConfig
                ...
                {
                    //For each binning variable, we extract the input column, type of binning and type associated
                    // parameter and then pass the binning transformer with the user specified parameters
                    val binningType = transformationObj(FlashMLConstants.BINNING_TYPE)
                            .asInstanceOf[String]
                    val inputCol = transformationObj(FlashMLConstants.INPUT_VARIABLE)
                            .asInstanceOf[String]
                            ...
                            .toLowerCase match
                    {
                        ...
                        case FlashMLConstants.BINNING_TYPE_EQUIDISTANT =>
                            val numberOfBuckets = transformationObj(FlashMLConstants.INPUT_PARAMETER)
                                    .asInstanceOf[Int]
                            ...
                            val splits: Array[Double] = (ArrayBuffer(Double.NegativeInfinity) ++ bucketIntervals ++
                        ...
                        case FlashMLConstants.BINNING_TYPE_INTERVALS =>
                            ...
                            new Bucketizer()
                                    .setInputCol(inputCol)
                                    .setOutputCol(outputCol)
                                    .setSplits((ArrayBuffer(Double.NegativeInfinity) ++ splits.to[ArrayBuffer] ++
                                            ArrayBuffer(Double.PositiveInfinity)).toArray)
                    }
                })
    ...
    private def getGramsPipelineStages(map: util.HashMap[String, Any]): ArrayBuffer[PipelineStage] =
    {
        ...
        log.info(s"Feature Generation: Adding Gram Assembler transformer. Columns added: [${intermediateColumnArray
                .mkString(",")}]")
    ...
    val loadPipelineModel: Int => PipelineModel = loadPipelineModel(_: Int, "featureGeneration")

    /**
     * Validating flashml featuregeneration configuration for checking whether the input variables are part of
     * preprocessing output variables
        ...
        if (featureGenerationGramsConfig.nonEmpty)
        {
            ...
            if (ConfigUtils.isPageLevelModel)
            {
                (0 to ConfigUtils.numPages - 1).foreach(pageNumber =>
                {
                    ...
                    {
                        case FlashMLConstants.SCOPE_PARAMETER_NO_PAGE | FlashMLConstants.SCOPE_PARAMETER_ALL_PAGE =>
                            ConfigUtils.scopeTextVariables.asInstanceOf[Array[String]]
                    ...
                    ConfigUtils.featureGenerationScope match
                    {
                        case FlashMLConstants.SCOPE_PARAMETER_NO_PAGE | FlashMLConstants.SCOPE_PARAMETER_ALL_PAGE =>
                                    ...
                                    .asInstanceOf[Array[util.HashMap[String, Any]]].nonEmpty)
                            {
                                ...
                                val featureGenerationGrams = featureGenerationGramsConfig.asInstanceOf[Array[util
                                .HashMap[String, Any]]]
                                // fetching valid preprocessing output variables
                                val preprocessingOutputVars = preprocessingStepsPageWise
                                        .asInstanceOf[Array[util.HashMap[String, Any]]]
                                        .filter(obj => textVariables.contains(obj.get(FlashMLConstants
                                                .INPUT_VARIABLE).toString))
                                        .map(obj =>
                                        {
                                            obj
                                                    .get(FlashMLConstants.OUTPUT_VARIABLE)
