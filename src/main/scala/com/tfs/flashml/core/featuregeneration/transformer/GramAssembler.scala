...
{
    def this() = this(Identifiable.randomUID("gram_assembler"))
    ...
    {
        // Data transformation.
        val assembleFunc = udf[Seq[String], Row]{ r: Row => GramAssembler.assemble(r.toSeq: _*) }
    ...
    override def transformSchema(schema: StructType): StructType =
    {
        ...
        $(inputCols) foreach
            { strCol: String =>
                ...
                if (!(field.dataType == ArrayType(StringType, false) || field.dataType == ArrayType(StringType, true)))
                {
                    throw new Exception(s"Input type ${field.dataType} did not match required type ArrayType(StringType)")
                }
            }
