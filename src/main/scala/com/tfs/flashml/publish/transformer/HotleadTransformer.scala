    ...
    with DefaultParamsWritable
{
  ...
  def transform(df: Dataset[_]): DataFrame =
  {
    ...
    { r: Row =>
      ...
      val top = if(r.length > 2) r.get(2).asInstanceOf[Double] else 0.0
      if(page >= $(nPages))
        if(prob >= $(thresholds).last && (if(r.length > 2){ if(top >= $(topThresholds).last) true else false } else true)) true else false
      else
        if(prob >= $(thresholds)(page-1) && (if(r.length > 2){ if(top >= $(topThresholds)(page-1)) true else false } else true)) true else false
    }

    val args: Array[Column] = $(inputCols).map
    { c => df(c) }

    var res = df.select(col("*"), isHotlead(struct(args: _*)).as($(outputCol)))
    res
  }
