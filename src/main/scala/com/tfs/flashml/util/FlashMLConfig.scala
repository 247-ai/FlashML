...
object FlashMLConfig
{
  ...
  val masterConfPath = getClass.getResource("/MasterConfig.json")
  ...
  /**
    ...
    */
  var config: Config = if (ConfigUtils.configFilePath.startsWith("hdfs"))
  {
    loadConfFromHDFS(ConfigUtils.configFilePath).withFallback(longList)
  }
  else
  {
    val configFile = new File(ConfigUtils.configFilePath)
    // Issue a warning if the config file is not found.
    if(!ConfigUtils.configFilePath.isEmpty && !configFile.exists())
  ...
  def getInt(key: String): Int = if(config.hasPath(key)) config.getInt(key) else Int.MinValue
  ...
  def getBool(key: String): Boolean = config.getBoolean(key)
