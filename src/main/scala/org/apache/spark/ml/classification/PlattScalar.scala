    ...
    with HasLabelCol
{
    ...
    {
        def rawPredExtractionUdf(idx: Int) = udf[Double, Vector]((a: Vector) => a(idx))
        ...
        with MLWritable
{
    ...
    override def transform(dataset: Dataset[_]): DataFrame =
    {
        ...
        if ($(isMultiIntent))
        {
            ...
            val initUDF = udf[Map[Int, Double]]{ () => Map[Int, Double]() }
            ...
            val aggregatedDataset = models
                ...
                .foldLeft[DataFrame](newDataset)
                {
                    case (df, (model, index)) =>
                        ...
                        val updateUDF = udf[Map[Int, Double], Map[Int, Double], Vector]
                        { (predictions: Map[Int, Double], prediction: Vector) => predictions + ((index, prediction(1))) }
            ...
            val mapToVectorUDF = udf[Vector, Map[Int, Double]]
            {
            ...
            // UDF to extract the label index as per Platt scores.
            val plattLabelIndexUDF = udf[Double, Map[Int, Double]]
            {
            ...
            aggregatedDataset
                ...
                .withColumn("prediction", plattLabelIndexUDF(col(accColName)), origMetadata)
                .drop(accColName)
        }
        else
        {
