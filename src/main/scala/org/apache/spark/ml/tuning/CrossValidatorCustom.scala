                            ...
                            Future[MulticlassMetrics]
                                    {
                                        ...
                                        logger.info(s"For fold: ${splitIndex + 1} with paramMap: ${paramMap
                                                .toSingleLineString}\n" +
                                                s" Accuracy: ${decimalFormat.format(combinedMetrics.accuracy)}\n" +
                                                s" Weighted Precision: ${decimalFormat.format(combinedMetrics
                                                        .weightedPrecision)}\n" +
                                                s" Weighted Recall: ${decimalFormat.format(combinedMetrics
                                                        .weightedRecall)}")
