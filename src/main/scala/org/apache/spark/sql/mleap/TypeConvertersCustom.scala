...
/**
  * Created a custom class to support columns with nullable field as true in struct field
  */
trait TypeConvertersCustom {
  ...
  def sparkToMleapDataShape(field: StructField,
    ...
    case ArrayType(_,true) => types.ListShape(field.nullable)
    case vu: VectorUDT =>
      val size = getVectorSize(dataset, field)
...
object TypeConvertersCustom extends TypeConvertersCustom {
