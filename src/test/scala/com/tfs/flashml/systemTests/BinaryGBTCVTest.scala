...
class BinaryGBTCVTest extends AnyFlatSpec {
    ...
    FlashMLConfig.config= ConfigFactory.load("binary_gbt_cv_test_config.json")
    ...
    println("Test case: Binary Gradient Boosted Trees Regression - CV")
    ...
    "The Binary GBT TrainAUROC" should "match" in {
        withClue("BinaryTrainAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.binaryGBTCV.trainAUROC").toDouble) {
                binaryEvaluator.setMetricName("areaUnderROC").evaluate(binaryPrediction(0))
            }
        }
    }

    "The Binary GBT TestAUROC" should "match" in {
        withClue("BinaryTestAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.binaryGBTCV.testAUROC").toDouble) {
