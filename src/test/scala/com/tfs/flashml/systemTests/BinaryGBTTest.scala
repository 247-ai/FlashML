...
class BinaryGBTTest extends AnyFlatSpec {
    ...
    FlashMLConfig.config= ConfigFactory.load("binary_gbt_test_config.json")
    ...
    println("Test case: Binary Gradient Boosted Trees")
    ...
    "The Binary GB Trees TrainAUROC" should "match" in {
        withClue("GBT Trees TrainAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.binaryGBT.trainAUROC").toDouble) {
    ...
    "The Binary GB Trees TestAUROC" should "match" in {
        withClue("GBT Trees TestAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.binaryGBT.testAUROC").toDouble) {
