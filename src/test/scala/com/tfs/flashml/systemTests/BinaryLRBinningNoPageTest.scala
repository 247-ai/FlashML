...
class BinaryLRBinningNoPageTest extends AnyFlatSpec {
    ...
    FlashMLConfig.config= ConfigFactory.load("binaryLR_binning_test_noPage.json")
    ...
    println("Test case: Binary Logistic Regression - Binning - No Page")
    ...
    "The BinaryTrainAUROC" should "match" in {
        withClue("BinaryTrainAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.BinningNoPage.trainAUROC").toDouble) {
    ...
    "The BinaryTestAUROC" should "match" in {
        withClue("BinaryTestAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.BinningNoPage.testAUROC").toDouble) {
