...
class BinaryLogisticTest extends AnyFlatSpec {
  ...
  FlashMLConfig.config= ConfigFactory.load("binary_test_config.json")
  ...
  println("Test case: Binary Logistic Regression")
  ...
  "The Binary LR TrainAUROC" should "match" in {
    withClue("Binary LR TrainAUROC: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.binaryLR.trainAUROC").toDouble) {
        binaryEvaluator.setMetricName("areaUnderROC").evaluate(binaryPrediction(0))
      }
    }
  }

  "The Binary LR TestAUROC" should "match" in {
    withClue("Binary LR TestAUROC: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.binaryLR.testAUROC").toDouble) {
