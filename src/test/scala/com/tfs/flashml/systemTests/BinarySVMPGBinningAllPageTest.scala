...
class BinarySVMPGBinningAllPageTest extends AnyFlatSpec {
    ...
    FlashMLConfig.config= ConfigFactory.load("binning_test_allPage_config.json")
    ...
    println("Test case: Single Intent SVM PG Binning - All Page")
    ...
    "The BinaryTrainAUROC" should "match" in {
        withClue("BinaryTrainAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.BinningAllPage.trainAUROC").toDouble) {
    ...
    "The BinaryTestAUROC" should "match" in {
        withClue("BinaryTestAUROC: ") {
            assertResult(configSolutionsKeyPair.getString("FlashMLTests.BinningAllPage.testAUROC").toDouble) {
