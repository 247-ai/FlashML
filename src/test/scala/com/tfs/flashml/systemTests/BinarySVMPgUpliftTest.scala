...
class BinarySVMPgUpliftTest extends FlatSpec {
  ...
  var svmSIPGUPPredictionDF: Array[RDD[(Double,Double)]] = SavePointManager.loadData(FlashMLConstants.SCORING).map(_.withColumn("positive_probability", ConfigUtils.pos_prob(col("modelProbability"))).select("positive_probability", ConfigUtils.getIndexedResponseColumn).as[(Double, Double)].rdd)
