...
class MultiIntentLRRandomSamplingTest extends AnyFlatSpec {
  ...
  println("Test Case: LR Multiple Intent Random Sampling")

  FlashMLConfig.config = ConfigFactory.load("multiIntent_lr_randomSampling_test_config.json")
  ...
  "LR-MultiIntent-Train-Precision" should "match" in {
    withClue("LR-MultiIntent-Train-Precision: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentLRRandom.trainPrecision").toDouble) {
  ...
  "LR-MultiIntent-Train-Recall" should "match" in {
    withClue("LR-MultiIntent-Train-Recall: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentLRRandom.trainRecall").toDouble) {
  ...
  "LR-MultiIntent-Test-Precision" should "match" in {
    withClue("LR-MultiIntent-Test-Precision: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentLRRandom.testPrecision").toDouble) {
  ...
  "LR-MultiIntent-Test-Recall" should "match" in {
    withClue("LR-MultiIntent-Test-Recall: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentLRRandom.testRecall").toDouble) {
