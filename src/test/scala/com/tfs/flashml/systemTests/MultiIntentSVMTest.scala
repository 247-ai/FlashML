...
class MultiIntentSVMTest extends FlatSpec{
  ...
  "SVM-MultiIntent -YELP - Train-Precision" should "match" in {
    withClue("SVM-MultiIntent - YELP -Train-Precision: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentSVM.trainPrecision").toDouble) { multiIntentSVMEvaluatorTrain.weightedPrecision
      }
    }
  }
  ...
  "SVM-MultiIntent -YELP -Test-Precision" should "match" in {
    withClue("SVM-MultiIntent- YELP -Test-Precision: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentSVM.testPrecision").toDouble) { multiIntentSVMEvaluatorTest.weightedPrecision
      }
  ...
  "SVM-MultiIntent - YELP -Test-Recall" should "match" in {
    withClue("SVM-MultiIntent- YELP  - Test-Recall: ") {
      assertResult(configSolutionsKeyPair.getString("FlashMLTests.multiIntentSVM.testRecall").toDouble) { multiIntentSVMEvaluatorTest.weightedRecall
      }
    }
  }
